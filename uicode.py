...
import KNN,mlp,pre_trained_classification,add_steps,gaussian,pre_trained_regression
...
class UI(QMainWindow):
    def __init__(self):
        ...
        self.con_hot_btn = self.findChild(QPushButton,"convert_btn2")
        ...
        self.fillna_btn.clicked.connect(self.fillmedian)
        ...
        self.con_hot_btn.clicked.connect(self.con_cat_hot)
        ...
        self.go_pre_trained1.clicked.connect(self.test_pretrained_classification)
        self.go_pre_trained.clicked.connect(self.test_pretrained_regression)
    ...
    def scale_value(self):
        ...
        steps.add_text(self.scaler.currentText()+" is applied to data")
    ...
    def test_pretrained_classification(self):

        self.testing=pre_trained_classification.UI(self.df,self.target_value,self.pickle_model,self.filePath_pre)


    def test_pretrained_regression(self):

        self.testing=pre_trained_regression.UI(self.df,self.target_value,self.pickle_model,self.filePath_pre)
    ...
    def con_cat_hot(self):


        self.df,func_name =data.convert_category_Onehot(self.df,data.get_cat(self.df))
        steps.add_text("Dtaframe converted using OneHotEncoder")
        steps.add_pipeline("Onehot Encoder",func_name)
        self.filldetails()
    ...
    def fillmedian(self):
        ...
        self.df[self.emptycolumn.currentText()]=data.fillmedian(self.df,self.emptycolumn.currentText())
        code="data['"+self.emptycolumn.currentText()+"'].fillna(data['"+self.emptycolumn.currentText()+"'].median(),inplace=True)"
        steps.add_code(code)
